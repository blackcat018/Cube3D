...
int get_map_value(t_game *g, int x, int y)
{
    ...
    if (g->my_map->map[y][x] == '1')
        return 1;
    if (g->my_map->map[y][x] == 'd' || g->my_map->map[y][x] == '0' || g->my_map->map[y][x] == 'N' || 
...
int fill_tex(t_pars	*my_map, t_game *g)
{
	...
	if (!(g->img_wall_west))
		return (1);
    g->texture_door= mlx_load_png("joj.png");
    if (!g->texture_door)
        return (1);
    g->img_wall_door = mlx_texture_to_image(g->mlx, g->texture_door);
	if (!(g->img_wall_door))
		return (1);
    return 0;
}
...
void    free_texturs(t_game *g)
{
    ...
    mlx_delete_texture(g->texture_door);
    ...
    mlx_delete_image(g->mlx, g->img_wall_door);
